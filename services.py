    ...
    if request.response is None:
        ...
        if exception is not None:
            ...
            print(
                BOLD
                + BG.red
                + f"{url.format(FG.lightred) + BOLD} Отправка не удалась!"
                + RESET_ALL,
            ...
            print(
                BOLD
                + BG.green
                + f"{url.format(FG.lightgreen) + BOLD} Успешно отправлено!"
                + RESET_ALL
            )


def services(phone: str, count: int):
